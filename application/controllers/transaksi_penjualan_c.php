	...
	function index()
	{
		...
		if($this->input->post('simpan')){
			...
			foreach ($id_produk as $key => $val) {
				$this->model->simpan_detail_penjualan($id_penjualan, $val, $kode_akun[$key], $nama_produk[$key], $qty[$key], $harga_modal[$key], $harga_jual[$key], $harga_invoice[$key], $tax[$key], $cashback[$key], $profit[$key]);	
